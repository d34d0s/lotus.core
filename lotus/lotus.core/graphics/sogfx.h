...
#define SOGL_LOGO_SIZE (ubyte4)217;

#define SOGL_BUFFER_VBO 0
#define SOGL_BUFFER_EBO 1
#define SOGL_BUFFER_VAO 2

typedef enum sogl_buffer_attr {
    SOGL_LOCATION_ATTR = 1 << 0, // 1 (0b0001)
    SOGL_COLOR_ATTR    = 1 << 1, // 2 (0b0010)
    SOGL_TCOORD_ATTR   = 1 << 2, // 4 (0b0100)
    SOGL_NORMAL_ATTR   = 1 << 3, // 8 (0b1000)
    SO_VERTEX_ATTRIBS  = 1 << 4 
} sogl_buffer_attr;

typedef struct sogl_buffer {
    ubyte4 handle[3];
    ubyte attrs;
    sogl_memory mem;
} sogl_buffer;

typedef struct sogl_pipeline {
    void* shader;
    void* sampler;
    struct bindings {
        void* vertex_buffer;
